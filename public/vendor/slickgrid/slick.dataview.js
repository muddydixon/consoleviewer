(function ($) {
  ...
  function DataView(options) {
    ...
    function group(groupObject) {
    }
    function groupBy(valueGetter, valueSetter, options) {
      if (!options.groupItemMetadataProvider) {
        options.groupItemMetadataProvider = new Slick.Data.GroupItemMetadataProvider();
      }

      groupingGetter = valueGetter;
      groupingSetter = valueSetter;
      groupingGetterIsAFn = typeof groupingGetter === "function";
      groupingFormatter = options.format || JSON.stringify;
      groupingComparer = options.sort;
      groupingFinalizer = options.end;
      collapsedGroups = {};
      groups = [];
      refresh();
    }
    ...
    function extractGroups(rows) {
      ...
      // performance test 20121101
      var groupingGetterFun = groupingGetter;
      if(!groupingGetterIsAFn){
          groupingGetterFun = function(r){
            return r[groupingGetter];
          };
      }
...
//         val = groupingGetterFun(r);
    ...
    function recalc(_items) {
      ...
      if(groupingGetter != null){
        ...
        if (groups.length) {
          var groupedRow = groups.map(function(g){
            g.collapsed = (g.value in collapsedGroups);
            g.title = groupingFormatter ? groupingFormatter(g) : g.value;
            return groupingSetter(g);
          });
          if (aggregators) {
...
//           newRows = flattenGroupedRows(groups);
          if(groupingFinalizer)
            groupingFinalizer(groupedRow);
        }
      }
